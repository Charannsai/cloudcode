...
export async function gitPush(containerId: string, remote = 'origin', branch?: string): Promise<string> {
  ...
  const exitCode = await execInContainer(containerId, ['sh', '-c', `cd ${WORKSPACE} && GIT_SSH_COMMAND="ssh -i /home/coder/.ssh/id_ed25519 -o StrictHostKeyChecking=no" git -c safe.directory=/workspace -c core.fileMode=false push ${remote} ${branchArg} 2>&1`], (data) => {
    output += data
  })
  if (exitCode !== 0) {
...
export async function gitPull(containerId: string, remote = 'origin', branch?: string): Promise<string> {
  ...
  let output = ''
  const branchArg = branch || ''
  const exitCode = await execInContainer(containerId, ['sh', '-c', `cd ${WORKSPACE} && GIT_SSH_COMMAND="ssh -i /home/coder/.ssh/id_ed25519 -o StrictHostKeyChecking=no" git -c safe.directory=/workspace -c core.fileMode=false pull ${remote} ${branchArg} 2>&1`], (data) => {
    output += data
  })
  if (exitCode !== 0) {
