...
const get3DTransform = (progress: number, isMobile: boolean, mouseX: number, mouseY: number) => {
  ...
  const diff = k2.p - k1.p;
  const factor = diff === 0 ? 0 : (progress - k1.p) / diff;

  const ease = factor * factor * (3 - 2 * factor);
  const lerp = (a: number, b: number) => a + (b - a) * ease;

  const scaleFactor = isMobile ? 0.25 : 1.0;

  const mouseRx = isMobile ? 0 : mouseY * -15;
  const mouseRy = isMobile ? 0 : mouseX * 15;

  const rx = (lerp(k1.rx, k2.rx) + mouseRx) * scaleFactor;
  const ry = (lerp(k1.ry, k2.ry) + mouseRy) * scaleFactor;
  const rz = lerp(k1.rz, k2.rz) * scaleFactor;
  const s = lerp(k1.s, k2.s) * (isMobile ? 0.8 : 1.0);
  const tx = lerp(k1.tx, k2.tx) * scaleFactor;
  const ty = lerp(k1.ty, k2.ty) * scaleFactor;
  const tz = lerp(k1.tz, k2.tz) * scaleFactor;
